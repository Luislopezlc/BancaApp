  ...
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (context) => ListMovementsCategoryBloc(
              LoadListMovementsCategoryData(
                  transfersRepository(), servicesRepository()),
            )..add(LoadMovementsCategoryDataEvent()),
        child: Scaffold(
          body: BlocListener<ListMovementsCategoryBloc,
                  ListMovementsCategoryState>(
              listener: (context, state) async {},
              child: BlocBuilder<ListMovementsCategoryBloc,
                  ListMovementsCategoryState>(builder: (context, state) {
                if (state is ListMovementsCategoryLoading ||
                    state is ListMovementsCategoryInitial) {
                  return Scaffold(
                      appBar: AppBar(
                ...
                }else if(state is ListMovementsCategoryLoaded)
                {
                    return buildHistoryCategoriesMovements(context, state.movements);
                }
                else if(state is ListMovementsCategoryError)
                {
                    return buildHistoryCategoriesMovements(context, []);
                }else{
                  return ErrorPage();
                }
...
Widget buildHistoryCategoriesMovements(
    BuildContext context, List<MovementsCategoryModel> movements) {
  return Scaffold(
      appBar: AppBar(
        backgroundColor: Color.fromRGBO(124, 77, 246, 1.000),
        title: const Text(
        ...
        child: SingleChildScrollView(
          child: Container(
            ...
            child: Column(
              ...
              children: movements.map((category) {
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // Categoría
